...
const SECRET_HINT = (
  <>
    Per attivare il modello imposta il segreto{' '}
    <code className="rounded bg-ink/8 px-1 py-0.5">wrangler pages secret put OPENAI_API_KEY</code> (oppure{' '}
    <code className="rounded bg-ink/8 px-1 py-0.5">MINIMAX_API_KEY</code>) sul progetto Cloudflare Pages e ridistribuisci
    (vedi README).
  </>
)
      ...
      >
        ...
        <div className="flex-1 space-y-1">
          <p className="font-medium">
            L&apos;API di trasmutazione non ha risposto: il risultato qui sotto proviene dal motore dimostrativo offline.
  ...
  if (fallback?.kind === 'unconfigured') {
    return (
      ...
      >
        ...
        <div className="flex-1 space-y-1">
          <p className="font-medium">
            Chiave del modello assente sul server: il risultato proviene dal motore dimostrativo offline.
            <span className="font-cjk text-ink-soft"> · 服务器未配置模型密钥，演示模式</span>
          </p>
          <p className="text-ink-soft font-mono text-xs break-all">{fallback.message}</p>
  ...
  return (
    ...
    >
      ...
      <div className="space-y-0.5">
        <p className="font-medium">
          Modalità dimostrativa: nessun modello linguistico è configurato, le trasmutazioni sono generate offline da un
          motore a modelli.
        </p>
        <p className="font-cjk text-ink-soft">演示模式：尚未配置语言模型，所有转化均由离线模板引擎生成。</p>
        <p className="text-ink-soft text-xs">{SECRET_HINT}</p>
      </div>
    </div>
  )
